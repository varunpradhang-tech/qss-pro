...
| P0 | QSS-SLAB-002 | Reference drawing must contain one closed P-panel polyline per accepted slab panel, label inside the panel, and quantity must read back from those marks. | Coded-audited but field behavior still misses P marks. | Make P-panel read-back mandatory before MB rows; every row must link to a P polyline handle/id. |
...
| P1 | QSS-SLAB-005 | P-panel numbering is left-to-right, top-to-bottom and never random. | Rulebook says documented-pending-full-code; no direct golden test. | Sort verified slab cells by top-to-bottom rows, then left-to-right inside each row before assigning P numbers. |
